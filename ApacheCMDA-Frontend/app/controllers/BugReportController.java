...
public class BugReportController extends Controller {
	...
	public static Result newReport() {
		...
		try {
			...
			e.printStackTrace();
			Application.flashMsg(adapter.createResponse(ResponseType.UNKNOWN));
		}	
	...
	public static Result deleteReport() {
		...
		try {
			...
			if(BugReport.delete(id)){
				Application.flashMsg(adapter.createResponse(ResponseType.SUCCESS));
			}else {
				Application.flashMsg(adapter.createResponse(ResponseType.DELETEERROR));
			}
		} catch (Exception e) {
			e.printStackTrace();
			Application.flashMsg(adapter.createResponse(ResponseType.UNKNOWN));
	...
	public static Result solveReport() {
		...
		try {
			...
			if(BugReport.solve(id)){
				...
				Application.flashMsg(adapter.createResponse(ResponseType.RESOLVEERROR));
			}
		} catch (Exception e) {
			e.printStackTrace();
			Application.flashMsg(adapter.createResponse(ResponseType.UNKNOWN));
